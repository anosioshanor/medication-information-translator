...
with st.sidebar:
    ...
    if history:
        ...
        for entry in recent_searches:
            ...
            with col1:
                if st.button(f"💊 {medication}", key=f"history_{medication}_{timestamp}"):
            ...
            with col2:
                ...
                if st.button("✕", key=f"del_{medication}_{timestamp}"):
...
default_value = st.session_state.get("medication_name", "")
